...
class FirebaseRepository {
    ...
    fun acceptFamilyInvite(userId: String, familyId: String, inviteId: String, callback: (Boolean, String?) -> Unit) {
        ...
        Log.d(TAG, "acceptFamilyInvite: userId=$userId, familyId=$familyId, inviteId=$inviteId")
        
        // Primeiro, buscar informações da família e do usuário que está aceitando
        firestore.collection(FAMILIES_COLLECTION).document(familyId)
            .get()
            .addOnSuccessListener { familyDoc ->
                val family = familyDoc.toObject(Family::class.java)
                if (family == null) {
                    callback(false, "Família não encontrada")
                    return@addOnSuccessListener
                }
                
                // Buscar dados do usuário que está aceitando o convite
                firestore.collection(USERS_COLLECTION).document(userId)
                    .get()
                    .addOnSuccessListener { userDoc ->
                        ...
                        firestore.collection(USERS_COLLECTION).document(family.responsibleId)
                            .get()
                            .addOnSuccessListener { responsibleDoc ->
                                val responsible = responsibleDoc.toObject(User::class.java)
                                if (responsible == null) {
                                    callback(false, "Responsável não encontrado")
                                    return@addOnSuccessListener
                                }
                                ...
                                firestore.collection(USERS_COLLECTION).document(userId)
                                    ...
                                    .addOnSuccessListener {
                                        ...
                                        val relationship = FamilyRelationship(
                                            dependentId = userId,
                                            responsibleId = family.responsibleId,
                                        ...
                                        saveFamilyRelationship(relationship) { success, exception ->
                                            if (success) {
                                                // Remover convite
                                                firestore.collection(INVITES_COLLECTION).document(inviteId)
                                                    .delete()
                                                    .addOnSuccessListener {
                                                        Log.d(TAG, "Convite aceito com sucesso. Usuário $userId adicionado à família $familyId e relacionamento criado")
                                                        callback(true, null)
                                                    }
                                                    .addOnFailureListener { e ->
                                                ...
                                                Log.e(TAG, "Erro ao criar relacionamento familiar", exception)
                                                callback(false, "Erro ao criar relacionamento: ${exception?.message}")
                                            }
                                        }
                                    }
                                    .addOnFailureListener { e ->
                                        Log.e(TAG, "Erro ao atualizar usuário", e)
                                        callback(false, "Erro ao atualizar usuário: ${e.message}")
                                    }
                            }
                            .addOnFailureListener { e ->
                                Log.e(TAG, "Erro ao buscar responsável", e)
                                callback(false, "Erro ao buscar responsável: ${e.message}")
                            }
                    }
                    .addOnFailureListener { e ->
                        Log.e(TAG, "Erro ao buscar usuário", e)
                        callback(false, "Erro ao buscar usuário: ${e.message}")
                    }
            }
            .addOnFailureListener { e ->
                Log.e(TAG, "Erro ao buscar família", e)
    ...
    fun getResponsibleForDependent(dependentId: String, onComplete: (FamilyRelationship?, Exception?) -> Unit) {
        ...
        // Primeiro, verificar se o dependente existe e tem familyId
        firestore.collection(USERS_COLLECTION).document(dependentId)
            .get()
            .addOnSuccessListener { userDoc ->
                val user = userDoc.toObject(User::class.java)
                if (user == null) {
                    Log.e(TAG, "[getResponsibleForDependent] Usuário dependente não encontrado: $dependentId")
                    onComplete(null, Exception("Usuário dependente não encontrado"))
                    return@addOnSuccessListener
                }
                
                Log.d(TAG, "[getResponsibleForDependent] Dados do dependente: name=${user.name}, familyId=${user.familyId}, type=${user.type}")
                
                if (user.familyId.isNullOrEmpty()) {
                    Log.w(TAG, "[getResponsibleForDependent] Dependente $dependentId não tem familyId")
                    onComplete(null, null)
                    return@addOnSuccessListener
                }
                
                // Buscar na coleção de relacionamentos
                firestore.collection(FAMILY_RELATIONSHIPS_COLLECTION)
                    ...
                    .get()
                    .addOnSuccessListener { documents ->
                        Log.d(TAG, "[getResponsibleForDependent] Documentos encontrados na coleção familyRelationships: ${documents.size()}")
                        
                        if (documents.isEmpty) {
                            Log.w(TAG, "[getResponsibleForDependent] Nenhum relacionamento encontrado para dependente $dependentId")
                            Log.d(TAG, "[getResponsibleForDependent] Tentando buscar responsável diretamente na família...")
                            
                            // Fallback: buscar responsável diretamente na família
                            firestore.collection(USERS_COLLECTION)
                                .whereEqualTo("familyId", user.familyId)
                                .whereEqualTo("type", "responsavel")
                                .limit(1)
                                .get()
                                .addOnSuccessListener { responsibleDocs ->
                                    if (responsibleDocs.isEmpty) {
                                        Log.w(TAG, "[getResponsibleForDependent] Nenhum responsável encontrado na família ${user.familyId}")
                                        onComplete(null, null)
                                    } else {
                                        val responsible = responsibleDocs.documents[0].toObject(User::class.java)
                                        Log.d(TAG, "[getResponsibleForDependent] Responsável encontrado diretamente: ${responsible?.name}")
                                        
                                        // Criar relacionamento automaticamente
                                        val relationship = FamilyRelationship(
                                        ...
                                        saveFamilyRelationship(relationship) { success, exception ->
                                            if (success) {
                                                Log.d(TAG, "[getResponsibleForDependent] Relacionamento criado automaticamente")
                                                onComplete(relationship, null)
                                            } else {
                                                Log.e(TAG, "[getResponsibleForDependent] Erro ao criar relacionamento automaticamente", exception)
                                ...
                                .addOnFailureListener { e ->
                                    Log.e(TAG, "[getResponsibleForDependent] Erro ao buscar responsável diretamente", e)
                                    onComplete(null, e)
                                }
                        } else {
            ...
            .addOnFailureListener { e ->
                Log.e(TAG, "[getResponsibleForDependent] Erro ao buscar usuário dependente $dependentId", e)
                onComplete(null, e)
            }
    }
    ...
    /**
     * Método para migrar relacionamentos familiares existentes
     * Cria relacionamentos para famílias que já existem mas não têm relacionamentos na coleção familyRelationships
     */
    fun migrateExistingFamilyRelationships(callback: (Boolean, String?) -> Unit) {
        Log.d(TAG, "Iniciando migração de relacionamentos familiares existentes")
        
        // Buscar todas as famílias
        firestore.collection(FAMILIES_COLLECTION)
            .get()
            .addOnSuccessListener { familyDocs ->
                var processedFamilies = 0
                var totalFamilies = familyDocs.size()
                var successCount = 0
                var errorCount = 0
                
                if (totalFamilies == 0) {
                    callback(true, "Nenhuma família encontrada para migração")
                    return@addOnSuccessListener
                }
                
                Log.d(TAG, "Encontradas $totalFamilies famílias para processar")
                ...
                for (familyDoc in familyDocs) {
                    ...
                    firestore.collection(USERS_COLLECTION)
                        ...
                        .addOnSuccessListener { userDocs ->
                            val members = userDocs.documents.mapNotNull { it.toObject(User::class.java) }
                            val dependents = members.filter { it.type == "membro" }
                            val responsible = members.find { it.type == "responsavel" }
                            
                            if (responsible != null && dependents.isNotEmpty()) {
                                Log.d(TAG, "Processando família ${family.name}: ${dependents.size} dependentes")
                                
                                var processedDependents = 0
                                val totalDependents = dependents.size
                                
                                for (dependent in dependents) {
                                    // Verificar se já existe relacionamento
                                    firestore.collection(FAMILY_RELATIONSHIPS_COLLECTION)
                                        .whereEqualTo("dependentId", dependent.id)
                                        .whereEqualTo("responsibleId", responsible.id)
                                        .get()
                                        .addOnSuccessListener { existingDocs ->
                                            if (existingDocs.isEmpty) {
                                                ...
                                                saveFamilyRelationship(relationship) { success, exception ->
                                                    ...
                                                    if (success) {
                                                        successCount++
                                                        Log.d(TAG, "Relacionamento criado para dependente ${dependent.name}")
                                                    } else {
                                                        errorCount++
                                                        Log.e(TAG, "Erro ao criar relacionamento para dependente ${dependent.name}", exception)
                                                    }
                                                    
                                                    if (processedDependents == totalDependents) {
                                                        processedFamilies++
                                                        if (processedFamilies == totalFamilies) {
                                                            val message = "Migração concluída: $successCount sucessos, $errorCount erros"
                                                            Log.d(TAG, message)
                                        ...
                                        .addOnFailureListener { e ->
                                            processedDependents++
                                            errorCount++
                                            Log.e(TAG, "Erro ao verificar relacionamento existente", e)
                                            if (processedDependents == totalDependents) {
                                                processedFamilies++
                                                if (processedFamilies == totalFamilies) {
                                                    val message = "Migração concluída: $successCount sucessos, $errorCount erros"
                        ...
                        .addOnFailureListener { e ->
                            processedFamilies++
                            errorCount++
                            Log.e(TAG, "Erro ao buscar membros da família ${family.name}", e)
                            if (processedFamilies == totalFamilies) {
                                val message = "Migração concluída: $successCount sucessos, $errorCount erros"
                                Log.d(TAG, message)
                                callback(true, message)
            ...
            .addOnFailureListener { e ->
                Log.e(TAG, "Erro ao buscar famílias para migração", e)
                callback(false, "Erro ao buscar famílias: ${e.message}")
            }
    }

    /**
     * Verifica e cria relacionamentos familiares automaticamente se necessário
     * Este método pode ser chamado para garantir que relacionamentos existam
     */
    fun ensureFamilyRelationshipsExist(familyId: String, callback: (Boolean, String?) -> Unit) {
        Log.d(TAG, "Verificando relacionamentos para família: $familyId")
            ...
            .addOnSuccessListener { familyDoc ->
                ...
                firestore.collection(USERS_COLLECTION)
                    ...
                    .addOnSuccessListener { userDocs ->
                        ...
                        if (responsible == null) {
                            callback(false, "Responsável não encontrado na família")
                            return@addOnSuccessListener
                        }
                        
                        if (dependents.isEmpty()) {
                            callback(true, "Nenhum dependente encontrado na família")
                            return@addOnSuccessListener
                        }
                        
                        Log.d(TAG, "Verificando relacionamentos: ${dependents.size} dependentes")
                                ...
                                .addOnSuccessListener { existingDocs ->
                                    if (existingDocs.isEmpty) {
                                        ...
                                        saveFamilyRelationship(relationship) { success, exception ->
                                            ...
                                            if (processedDependents == totalDependents) {
                                                val message = "Verificação concluída: $createdRelationships relacionamentos criados"
                                                Log.d(TAG, message)
                                                callback(true, message)
                                            }
                                        }
                                        ...
                                        processedDependents++
                                        Log.d(TAG, "Relacionamento já existe para dependente ${dependent.name}")
                                        if (processedDependents == totalDependents) {
                                            val message = "Verificação concluída: $createdRelationships relacionamentos criados"
                                            Log.d(TAG, message)
                                            callback(true, message)
                    ...
                    .addOnFailureListener { e ->
                        Log.e(TAG, "Erro ao buscar membros da família", e)
                        callback(false, "Erro ao buscar membros: ${e.message}")
                    }
            }
            .addOnFailureListener { e ->
                Log.e(TAG, "Erro ao buscar família", e)
